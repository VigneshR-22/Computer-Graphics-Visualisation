...
void InitFountain(void) // try to add a stop function
{
...
if(flag==0&&col==0)
	glColor3f(1,1,1);
else if(flag==0&&col==1)	glColor3f(1,0,0);
else if(flag==0&&col==2)	glColor3f(1,1,1);
else if(flag==0&&col==3)	glColor3f(0,1,1);
else if(flag==0&&col==4)	glColor3f(0,1,0);
else if(flag==0&&col==5)    glColor3f(1,1,0);
else if(flag==1)
    glColor3fv(X);
else if(flag==2)
...
void NormalKey(GLubyte key, GLint x, GLint y)
{
	...
	switch ( key )
	{
        ...
        case 'r':	case 'R': col=1;  glutPostRedisplay();
				break;
        case 'w':	case 'W': col=2;  glutPostRedisplay();
				break;
        case 'c':	case 'C': col=3;  glutPostRedisplay();
				break;
        case 'g':	case 'G': col=4;  glutPostRedisplay();
				break;
		case  'p':  case 'P': col=5;  	glutPostRedisplay();
...
void cover()
{
	...
	DrawTextXY(-2.5,1.2,0.0,0.001,"     SHAMBHAVI ");
	DrawTextXY(1,1.2,0.0,0.001,"        HARITH G R ");
	glColor3f(0.7,0.8,0.6);
	DrawTextXY(-2.55,0.95,0.0,0.001,"     (1EP16CS129) ");
	DrawTextXY(0.95,0.95,0.0,0.001,"        (1EP16CS032) ");

	glColor3f(0.7,0.6,0.1);
	DrawTextXY(-3.25,0,0.0,0.0007,"     Under the guidance of : ");
	glColor3f(1.0,0.8,0.4);
	DrawTextXY(-3.25,-.2,0.0,0.001,"    Mrs.SMITHA S P");
	DrawTextXY(-3.05,-.4,0.0,0.0007,"   ASSISTANT PROFESSOR ");
    DrawTextXY(1,-.2,0.0,0.001,"    Mr. MADHU R");
	DrawTextXY(1.20,-.4,0.0,0.0007,"   ASSISTANT PROFESSOR");
	DrawTextXY(-3.25,-.7,0.0,0.001,"       EAST POINT COLLEGE OF ENGINEERING AND TECHNOLOGY");
